...
SELECT name FROM people WHERE name = 'Eleni Margariti';
...
SELECT name FROM people WHERE id IN (21, 22);
-- 9. The cinema has just heard that they will be holding an exclusive midnight showing of 'Guardians of the Galaxy 2'!! Create a new entry in the 'movies' table to reflect this.
INSERT INTO movies (title, year, show_time) VALUES ('Guardians of the Galaxy 2', 2016, '00:00 - 2:00');
SELECT * FROM movies;
-- 10. The cinema would also like to make the Guardian movies a back to back feature. Update the 'Guardians of the Galaxy' show time from 18:55 to 21:30
UPDATE movies SET show_time = '18:55 - 21:50' WHERE title = 'Guardians of the Galaxy';

-- ## Research
--
-- 1. Research how to delete multiple entries from your table in a single command.
DELETE FROM `table` WHERE id IN (12, 14, 16);
DELETE FROM `table` WHERE id IN (SELECT * FROM table WHERE id = 12 )
DELETE FROM 'table' where id between 12 and 20
DELETE FROM `table` WHERE id >=12 and id<= 20
